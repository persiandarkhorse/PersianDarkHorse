...
export default function Onboarding() {
  ...
  function createAccount(event: React.FormEvent) {
    ...
    localStorage.setItem("fezi-account", JSON.stringify({ name: name.trim() || email.split("@")[0], email: email.trim(), selectedAgent: selected.id, createdAt: Date.now() }));
    navigate("/");
  }
