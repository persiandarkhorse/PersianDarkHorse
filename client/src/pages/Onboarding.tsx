...
export default function Onboarding() {
  ...
  const bindingById = useMemo(() => new Map((capabilityBindings.data ?? []).map((binding) => [binding.capability.id, binding])), [capabilityBindings.data]);
  const subscriptionQuery = trpc.subscription.me.useQuery(undefined, { retry: false });
  const canSelectAgent = (agentId: string) => subscriptionQuery.data ? Boolean(subscriptionQuery.data.isAdmin || (subscriptionQuery.data.source === "subscription" && subscriptionQuery.data.plan.agents.includes(agentId))) : true;
