      ...
      <div className="mx-auto flex min-h-screen max-w-[1560px] overflow-hidden bg-[#ffffff] shadow-[0_20px_80px_rgba(65,45,35,0.08)] lg:min-h-[calc(100vh-32px)] lg:my-4 lg:rounded-[30px]">
        ...
        <main className="flex min-h-screen min-w-0 flex-1 flex-col lg:min-h-0">
          <header className="relative flex h-[78px] items-center justify-between border-b border-[#eeeeee] px-5 md:px-9">
            ...
            {agentDrawerOpen && <div className="absolute right-5 top-[64px] z-40 w-[300px] rounded-3xl border border-[#ddd] bg-white p-4 shadow-2xl"><div className="flex items-center justify-between"><div><p className="text-sm font-semibold">{language === "fa" ? "انتخاب Agent" : "Choose an Agent"}</p><p className="mt-1 text-[10px] text-[#888]">{language === "fa" ? "پروفایل‌ها و نقش‌های آماده" : "Profiles and roles"}</p></div><Link href="/welcome" className="text-[10px] underline">{language === "fa" ? "مدیریت" : "Manage"}</Link></div><div className="mt-4 space-y-2">{["مانیکا", "ایجنت دوم", "ایجنت سوم", "ایجنت چهارم", "ایجنت پنجم"].map((agent, index) => <button key={agent} onClick={() => { setAgentDrawerOpen(false); if (index === 0) setActiveMode(modes[0]); }} className={`flex w-full items-center gap-3 rounded-2xl border p-3 text-right hover:bg-[#f7f7f5] ${index === 0 ? "border-black" : "border-[#eee]"}`}><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-black text-white"><Sparkles size={15} /></span><span className="flex-1"><span className="block text-xs font-semibold">{agent}</span><span className="mt-1 block text-[10px] text-[#888]">{index < 2 ? (language === "fa" ? "رایگان" : "Free") : (language === "fa" ? "پولی" : "Paid")}</span></span>{index === 0 && <Check size={15} />}</button>)}</div></div>}
