      ...
      <div className="mx-auto flex min-h-screen max-w-[1560px] overflow-hidden bg-[#ffffff] shadow-[0_20px_80px_rgba(65,45,35,0.08)] lg:min-h-[calc(100vh-32px)] lg:my-4 lg:rounded-[30px]">
        ...
        <main className="flex min-h-screen min-w-0 flex-1 flex-col lg:min-h-0">
          <header className="relative flex h-[78px] items-center justify-between border-b border-[#eeeeee] px-5 md:px-9">
            ...
            <div className="flex items-center gap-1 text-[#666666]"><button onClick={() => setAgentDrawerOpen(true)} className="flex items-center gap-2 rounded-xl px-3 py-2 text-[11px] hover:bg-[#f5f5f5]" aria-expanded={agentDrawerOpen} aria-controls="fezi-agent-picker" title="تغییر Agent"><img src={activeAgent.image} alt={`تغییر Agent، ${activeAgent.name}`} className="h-7 w-7 rounded-lg object-cover" />{activeAgent.name}</button><button onClick={() => setLeftNavOpen(true)} className="rounded-xl p-2.5 hover:bg-[#f5f5f5]" title="باز کردن منوی کاربری" aria-label="باز کردن منوی کاربری" aria-expanded={leftNavOpen} aria-controls="fezi-user-panel"><Menu size={19} /></button></div>
