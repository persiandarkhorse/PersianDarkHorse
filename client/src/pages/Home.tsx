...
const providerProfiles: Record<string, string[]> = {
  ...
  "Manus WebDev": ["Production Web", "Interactive MVP", "Landing Page", "Dashboard"],
  "React + Tailwind": ["Accessible UI", "Editorial UI", "Dashboard", "Mobile-first"],
  "Expo / Flutter": ["Mobile MVP", "Native Navigation", "Offline-first", "Production App"],
  "Typst": ["Academic", "Business Report", "Technical", "Persian RTL"],
  "FEZI Prompt Skill · Jasper-style": ["Campaign", "Cinematic", "Technical", "SEO / GEO"],
};
const providerSettingNotes: Record<string, string> = {
  "Seedance 2.0": "کنترل حرکت، حرکت دوربین و seed برای تکرارپذیری شات.",
  "Kling AI": "کنترل شدت حرکت و میزان وفاداری به تصویر مرجع.",
  "Veo": "کارگردانی سینمایی، صدا و سبک روایت.",
  "Runway": "شدت استایل و حرکت برای خروجی تبلیغاتی.",
  "Nano Banana": "وفاداری چهره، سبک و پس‌زمینه برای ویرایش دقیق.",
  "ChatGPT Image": "استایل، جزئیات و میزان پایبندی به دستور متنی.",
  "Gemma Image": "پروفایل سریع، سبک تصویر و محدودیت محتوایی.",
  "Manus WebDev": "Stack، احراز هویت، دیتابیس و حالت انتشار.",
  "Typst": "قالب PDF، راست‌چین فارسی و سبک ارجاع‌دهی.",
};
...
export default function Home() {
  ...
  const [creationSize, setCreationSize] = useState("1024×1024");
  const [creationResolution, setCreationResolution] = useState("1080p");
  const [creationFps, setCreationFps] = useState("30 FPS");
  const [creationAudio, setCreationAudio] = useState("با صدا");
  const [creationPageSize, setCreationPageSize] = useState("A4");
  const [creationOrientation, setCreationOrientation] = useState("عمودی");
  const [creationDocType, setCreationDocType] = useState("DOCX");
  const [creationPlatform, setCreationPlatform] = useState("وب / مرورگر");
  const [creationStack, setCreationStack] = useState("React + Tailwind");
  const [creationPromptStyle, setCreationPromptStyle] = useState("حرفه‌ای و دقیق");
  const [creationPromptLength, setCreationPromptLength] = useState("کامل");
  const [advancedProfile, setAdvancedProfile] = useState("استاندارد FEZI");
  const [advancedMotion, setAdvancedMotion] = useState("متوسط");
  const [advancedCamera, setAdvancedCamera] = useState("حرکت نرم سینمایی");
  const [advancedFidelity, setAdvancedFidelity] = useState("بالا");
  const [advancedStyle, setAdvancedStyle] = useState("طبیعی");
  const [advancedBackground, setAdvancedBackground] = useState("واقعی");
  const [advancedSeed, setAdvancedSeed] = useState("تصادفی");
  const [advancedDeployment, setAdvancedDeployment] = useState("پیش‌نمایش");
  const [advancedTemplate, setAdvancedTemplate] = useState("حرفه‌ای");
  const [advancedStructure, setAdvancedStructure] = useState("ساختاریافته");
  const [presetName, setPresetName] = useState("");
  const [savedPresets, setSavedPresets] = useState<CreationPreset[]>(() => { try { return JSON.parse(localStorage.getItem("fezi-creation-presets") ?? "[]") as CreationPreset[]; } catch { return []; } });
  const [imageGenerationSettings, setImageGenerationSettings] = useState({ ratio: "1:1", quality: "High", duration: "10 ثانیه", size: "1024×1024", fidelity: "بالا", style: "طبیعی", background: "واقعی", renderDetail: "متعادل" });
  ...
  function saveCurrentPreset() {
    if (!creationSettings || !presetName.trim()) return;
    const preset: CreationPreset = { id: `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`, name: presetName.trim(), toolId: creationSettings.toolId, provider: creationSettings.provider, ...currentPresetData() };
    setSavedPresets((current) => [preset, ...current.filter((item) => !(item.name === preset.name && item.toolId === preset.toolId && item.provider === preset.provider))]);
    setPresetName("");
  }

  function applyPreset(preset: CreationPreset) {
    setCreationRatio(preset.ratio); setCreationQuality(preset.quality); setCreationDuration(preset.duration); setCreationFormat(preset.format); setCreationSize(preset.size); setCreationResolution(preset.resolution); setCreationFps(preset.fps); setCreationAudio(preset.audio); setCreationPageSize(preset.pageSize); setCreationOrientation(preset.orientation); setCreationDocType(preset.docType); setCreationPlatform(preset.platform); setCreationStack(preset.stack); setCreationPromptStyle(preset.promptStyle); setCreationPromptLength(preset.promptLength); setAdvancedProfile(preset.advancedProfile); setAdvancedMotion(preset.motion); setAdvancedCamera(preset.camera); setAdvancedFidelity(preset.fidelity); setAdvancedStyle(preset.style); setAdvancedBackground(preset.background); setAdvancedSeed(preset.seed); setAdvancedDeployment(preset.deployment); setAdvancedTemplate(preset.template); setAdvancedStructure(preset.structure);
  }
  ...
  function applyCreationSettings() {
    ...
    const configs: Record<string, string> = {
      video: `نسبت تصویر: ${creationRatio} | رزولوشن: ${creationResolution} | کیفیت: ${creationQuality} | مدت: ${creationDuration} | فریم‌ریت: ${creationFps} | صدا: ${creationAudio} | خروجی: ${creationFormat}`,
      file: `نوع فایل: ${creationDocType} | کیفیت محتوا: ${creationQuality} | خروجی: ${creationFormat}`,
      website: `پلتفرم: ${creationPlatform} | استک: ${creationStack} | نسبت پیش‌نمایش: ${creationRatio} | کیفیت طراحی: ${creationQuality}`,
      app: `پلتفرم: ${creationPlatform} | استک: ${creationStack} | کیفیت معماری: ${creationQuality} | خروجی: ${creationFormat}`,
      image: `نسبت تصویر: ${creationRatio} | اندازه: ${creationSize} | کیفیت: ${creationQuality} | خروجی: ${creationFormat}`,
      pdf: `اندازه صفحه: ${creationPageSize} | جهت: ${creationOrientation} | کیفیت: ${creationQuality} | خروجی: PDF`,
      prompt: `سبک پرامپت: ${creationPromptStyle} | طول: ${creationPromptLength} | خروجی: ${creationFormat}`,
    };
    const advanced = `پروفایل موتور: ${advancedProfile} | حرکت: ${advancedMotion} | دوربین: ${advancedCamera} | وفاداری: ${advancedFidelity} | سبک: ${advancedStyle} | پس‌زمینه: ${advancedBackground} | Seed: ${advancedSeed} | انتشار: ${advancedDeployment} | قالب: ${advancedTemplate} | ساختار: ${advancedStructure}`;
    const config = `${configs[creationSettings.toolId] ?? "تنظیمات پیش‌فرض FEZI"} | تنظیمات اختصاصی موتور: ${advanced}`;
    if (creationSettings.toolId === "image") {
      setImageGenerationSettings({ ratio: creationRatio, quality: creationQuality, duration: creationDuration, size: creationSize, fidelity: advancedFidelity, style: advancedStyle, background: advancedBackground, renderDetail: advancedSeed });
  ...
  function switchAgent(index: number, direction: "left" | "right" = "left") {
    setAgentSlideDirection(direction);
  ...
  async function generateManikaImage() {
    ...
    try {
      const result = await imageMutation.mutateAsync({
        ...
        fidelity: imageGenerationSettings.fidelity,
        visualStyle: imageGenerationSettings.style,
        background: imageGenerationSettings.background,
        renderDetail: imageGenerationSettings.renderDetail,
        prompt,
  ...
  return (
    <div className="min-h-[100dvh] bg-[#ffffff] text-[#111111] selection:bg-[#e5e5e5]/50">
      <div className="mx-auto flex min-h-[100dvh] max-w-[1560px] bg-[#ffffff] shadow-[0_20px_80px_rgba(65,45,35,0.08)] lg:min-h-[calc(100vh-32px)] lg:my-4 lg:overflow-hidden lg:rounded-[30px]">
        ...
        <main className="flex min-h-[100dvh] min-w-0 flex-1 flex-col lg:min-h-0">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-5 sm:px-5 sm:py-8 md:px-12 lg:px-20">
            <div className="mx-auto max-w-3xl">
              <div className="mb-7 flex items-center gap-3 border-b border-[#eeeeee] pb-6 sm:mb-10 sm:gap-4 sm:pb-8">
                <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-[18px] bg-[#000000] shadow-md shadow-[#8f6f5c]/15 sm:h-16 sm:w-16 sm:rounded-[22px]"><img src={activeAgent.image} alt={`پرترهٔ ${activeAgent.name}`} className="h-full w-full object-cover object-top" /></div>
                <div><p className="font-serif text-xl font-semibold tracking-tight sm:text-2xl">سلام، من {activeAgent.name} هستم.</p><p className="mt-1 text-xs leading-5 text-[#666666] sm:text-sm">{activeAgent.detail}</p></div>
              </div>
              ...
              <div className="space-y-7">
                {messages.map((message) => <div key={message.id} className={`group flex gap-3 ${message.role === "user" ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`min-w-0 max-w-[92%] sm:max-w-[85%] ${message.role === "user" ? "text-left" : ""}`}>
          ...
          <div className="fezi-safe-bottom px-3 pb-3 pt-2 sm:px-5 sm:pb-5 md:px-12 lg:px-20">
            <div className="mx-auto max-w-3xl">
              {creationSettings && <div className="mb-3 rounded-[22px] border border-black/15 bg-white p-4 shadow-[0_12px_30px_rgba(0,0,0,.08)]" role="dialog" aria-label="تنظیمات تخصصی ابزار"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-semibold">تنظیمات {toolCatalog.find((item) => item.id === creationSettings.toolId)?.label}</p><p className="mt-1 text-[10px] text-[#777]">موتور انتخاب‌شده: <strong>{creationSettings.provider}</strong></p><p className="mt-1 text-[10px] leading-5 text-[#888]">{creationToolNotes[creationSettings.toolId]}</p></div><button onClick={() => setCreationSettings(null)} className="rounded-lg p-1.5 text-[#777] hover:bg-[#f2f2f0]" aria-label="بستن تنظیمات"><X size={15} /></button></div><div className="mt-4 grid gap-3 sm:grid-cols-2">{creationSettings.toolId === "video" && <><label className="text-[10px] font-medium">نسبت تصویر<select value={creationRatio} onChange={(event) => setCreationRatio(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs">{["16:9", "9:16", "1:1", "4:5"].map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-[10px] font-medium">رزولوشن<select value={creationResolution} onChange={(event) => setCreationResolution(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs">{["720p", "1080p", "4K"].map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-[10px] font-medium">مدت ویدیو<select value={creationDuration} onChange={(event) => setCreationDuration(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs">{["5 ثانیه", "10 ثانیه", "15 ثانیه", "30 ثانیه", "60 ثانیه"].map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-[10px] font-medium">خروجی<select value={creationFormat} onChange={(event) => setCreationFormat(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>MP4</option><option>WEBM</option><option>MOV</option></select></label><label className="text-[10px] font-medium">فریم‌ریت<select value={creationFps} onChange={(event) => setCreationFps(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>24 FPS</option><option>30 FPS</option><option>60 FPS</option></select></label><label className="text-[10px] font-medium">صدا<select value={creationAudio} onChange={(event) => setCreationAudio(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>با صدا</option><option>بدون صدا</option></select></label></>}{creationSettings.toolId === "image" && <><label className="text-[10px] font-medium">نسبت تصویر<select value={creationRatio} onChange={(event) => setCreationRatio(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs">{["1:1", "16:9", "9:16", "4:5", "4:3"].map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-[10px] font-medium">اندازه تصویر<select value={creationSize} onChange={(event) => setCreationSize(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs">{["512×512", "1024×1024", "1536×1024", "1024×1536", "2048×2048"].map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-[10px] font-medium">کیفیت<select value={creationQuality} onChange={(event) => setCreationQuality(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs">{["Draft", "Standard", "High", "Ultra"].map((value) => <option key={value}>{value}</option>)}</select></label><label className="text-[10px] font-medium">فرمت<select value={creationFormat} onChange={(event) => setCreationFormat(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>PNG</option><option>JPG</option><option>WEBP</option></select></label></>}{creationSettings.toolId === "file" && <><label className="text-[10px] font-medium">نوع سند<select value={creationDocType} onChange={(event) => setCreationDocType(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>DOCX</option><option>Markdown</option><option>TXT</option><option>CSV</option><option>ZIP</option></select></label><label className="text-[10px] font-medium">فرمت خروجی<select value={creationFormat} onChange={(event) => setCreationFormat(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>DOCX</option><option>PDF</option><option>MD</option><option>TXT</option></select></label><label className="text-[10px] font-medium">کیفیت محتوا<select value={creationQuality} onChange={(event) => setCreationQuality(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>پیش‌نویس</option><option>استاندارد</option><option>حرفه‌ای</option></select></label></>}{creationSettings.toolId === "pdf" && <><label className="text-[10px] font-medium">اندازه صفحه<select value={creationPageSize} onChange={(event) => setCreationPageSize(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>A4</option><option>A5</option><option>Letter</option><option>Legal</option></select></label><label className="text-[10px] font-medium">جهت صفحه<select value={creationOrientation} onChange={(event) => setCreationOrientation(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>عمودی</option><option>افقی</option></select></label><label className="text-[10px] font-medium">کیفیت PDF<select value={creationQuality} onChange={(event) => setCreationQuality(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>استاندارد</option><option>چاپی</option><option>بالا</option></select></label><div className="rounded-xl bg-[#f5f5f3] px-3 py-2 text-[10px] text-[#777]">خروجی نهایی همیشه PDF خواهد بود.</div></>}{(creationSettings.toolId === "website" || creationSettings.toolId === "app") && <><label className="text-[10px] font-medium">پلتفرم<select value={creationPlatform} onChange={(event) => setCreationPlatform(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>وب / مرورگر</option><option>Android</option><option>iOS</option><option>Desktop</option></select></label><label className="text-[10px] font-medium">Stack / فناوری<select value={creationStack} onChange={(event) => setCreationStack(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>React + Tailwind</option><option>React + Node.js</option><option>Expo</option><option>Flutter</option><option>HTML + CSS + JS</option></select></label><label className="text-[10px] font-medium">کیفیت معماری<select value={creationQuality} onChange={(event) => setCreationQuality(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>سریع / MVP</option><option>استاندارد</option><option>Production-ready</option></select></label></>}{creationSettings.toolId === "prompt" && <><label className="text-[10px] font-medium">سبک پرامپت<select value={creationPromptStyle} onChange={(event) => setCreationPromptStyle(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>حرفه‌ای و دقیق</option><option>سینمایی</option><option>تبلیغاتی</option><option>فنی</option></select></label><label className="text-[10px] font-medium">طول خروجی<select value={creationPromptLength} onChange={(event) => setCreationPromptLength(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>کوتاه</option><option>کامل</option><option>ساختاریافته و چندمرحله‌ای</option></select></label><label className="text-[10px] font-medium">فرمت پرامپت<select value={creationFormat} onChange={(event) => setCreationFormat(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>متن ساده</option><option>JSON</option><option>Markdown</option></select></label></>}</div><div className="mt-4 rounded-2xl border border-[#e7e7e4] bg-[#fafaf8] p-3"><div className="flex items-center justify-between gap-2"><p className="text-[10px] font-semibold">تنظیمات اختصاصی موتور</p><span className="rounded-full bg-black px-2 py-1 text-[9px] text-white">Preset</span></div><div className="mt-2 flex flex-col gap-2 sm:flex-row"><select value="" onChange={(event) => { const preset = savedPresets.find((item) => item.id === event.target.value); if (preset) applyPreset(preset); }} className="h-9 min-w-0 flex-1 rounded-xl border border-[#ddd] bg-white px-2 text-[10px]"><option value="">بارگذاری Preset ذخیره‌شده...</option>{savedPresets.filter((item) => item.toolId === creationSettings.toolId && item.provider === creationSettings.provider).map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><input value={presetName} onChange={(event) => setPresetName(event.target.value)} placeholder="نام Preset جدید" className="h-9 min-w-0 flex-1 rounded-xl border border-[#ddd] bg-white px-2 text-[10px]" /><button onClick={saveCurrentPreset} disabled={!presetName.trim()} className="rounded-xl bg-black px-3 text-[10px] text-white disabled:opacity-40">ذخیره</button></div><div className="mt-2 flex flex-wrap gap-1.5">{savedPresets.filter((item) => item.toolId === creationSettings.toolId && item.provider === creationSettings.provider).map((item) => <span key={`saved-${item.id}`} className="inline-flex items-center gap-1 rounded-full border border-[#ddd] bg-white px-2 py-1 text-[9px] text-[#555]"><button onClick={() => applyPreset(item)} className="hover:text-black">{item.name}</button><button onClick={() => setSavedPresets((current) => current.filter((candidate) => candidate.id !== item.id))} className="text-[#999] hover:text-red-600" aria-label={`حذف Preset ${item.name}`}>×</button></span>)}</div><p className="mt-1 text-[10px] leading-5 text-[#888]">{providerSettingNotes[creationSettings.provider] ?? "پروفایل و کنترل‌های مناسب این موتور را انتخاب کنید."}</p><label className="mt-3 block text-[10px] font-medium">پروفایل موتور<select value={advancedProfile} onChange={(event) => setAdvancedProfile(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs">{(providerProfiles[creationSettings.provider] ?? ["استاندارد FEZI", "کیفیت بالا", "سریع", "تخصصی"]).map((value) => <option key={value}>{value}</option>)}</select></label>{creationSettings.toolId === "video" && <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-[10px] font-medium">شدت حرکت<select value={advancedMotion} onChange={(event) => setAdvancedMotion(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>کم</option><option>متوسط</option><option>زیاد</option><option>بسیار زیاد</option></select></label><label className="text-[10px] font-medium">حرکت دوربین<select value={advancedCamera} onChange={(event) => setAdvancedCamera(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>ثابت</option><option>حرکت نرم سینمایی</option><option>پن / تیلت</option><option>دنبال‌کردن سوژه</option><option>اوربیت ۳۶۰ درجه</option></select></label><label className="text-[10px] font-medium">Seed / تکرارپذیری<select value={advancedSeed} onChange={(event) => setAdvancedSeed(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>تصادفی</option><option>قفل برای تکرار</option><option>تولید چند Seed</option></select></label><label className="text-[10px] font-medium">صدا و دیالوگ<select value={creationAudio} onChange={(event) => setCreationAudio(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>با صدا</option><option>بدون صدا</option><option>دیالوگ هماهنگ</option></select></label></div>}{creationSettings.toolId === "image" && <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-[10px] font-medium">وفاداری به مرجع<select value={advancedFidelity} onChange={(event) => setAdvancedFidelity(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>کم</option><option>متوسط</option><option>بالا</option><option>حداکثری</option></select></label><label className="text-[10px] font-medium">سبک تصویر<select value={advancedStyle} onChange={(event) => setAdvancedStyle(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>طبیعی</option><option>سینمایی</option><option>ادیتوریال</option><option>تبلیغاتی</option><option>تصویرسازی</option></select></label><label className="text-[10px] font-medium">پس‌زمینه<select value={advancedBackground} onChange={(event) => setAdvancedBackground(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>واقعی</option><option>استودیویی</option><option>شفاف</option><option>حذف پس‌زمینه</option></select></label><label className="text-[10px] font-medium">جزئیات رندر<select value={advancedSeed} onChange={(event) => setAdvancedSeed(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>سریع</option><option>متعادل</option><option>جزئیات حداکثری</option></select></label></div>}{(creationSettings.toolId === "website" || creationSettings.toolId === "app") && <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-[10px] font-medium">حالت استقرار<select value={advancedDeployment} onChange={(event) => setAdvancedDeployment(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>پیش‌نمایش</option><option>MVP قابل اجرا</option><option>Production-ready</option></select></label><label className="text-[10px] font-medium">داده و احراز هویت<select value={advancedStructure} onChange={(event) => setAdvancedStructure(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>بدون دیتابیس</option><option>Auth + Database</option><option>API + Auth + Database</option></select></label></div>}{(creationSettings.toolId === "pdf" || creationSettings.toolId === "file") && <div className="mt-3 grid gap-3 sm:grid-cols-2"><label className="text-[10px] font-medium">قالب حرفه‌ای<select value={advancedTemplate} onChange={(event) => setAdvancedTemplate(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>حرفه‌ای</option><option>آکادمیک</option><option>گزارش مدیریتی</option><option>فارسی راست‌چین</option></select></label><label className="text-[10px] font-medium">ساختار محتوا<select value={advancedStructure} onChange={(event) => setAdvancedStructure(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>ساختاریافته</option><option>خلاصه و اجرایی</option><option>با فهرست و ارجاع</option></select></label></div>}{creationSettings.toolId === "prompt" && <div className="mt-3"><label className="text-[10px] font-medium">فرمت ساختاریافته<select value={advancedStructure} onChange={(event) => setAdvancedStructure(event.target.value)} className="mt-1 h-10 w-full rounded-xl border border-[#ddd] bg-white px-2 text-xs"><option>ساختاریافته</option><option>JSON قابل استفاده در API</option><option>Markdown با بخش‌بندی</option></select></label></div>}</div><div className="mt-4 flex items-center justify-between gap-3"><span className="text-[10px] text-[#888]">این تنظیمات همراه درخواست به {activeAgent.name} ارسال می‌شود.</span><button onClick={applyCreationSettings} className="rounded-xl bg-black px-4 py-2.5 text-xs font-semibold text-white hover:bg-[#222]">تأیید و ادامه</button></div></div>}
